...
export class utils {
   ...
   */
  static async handleTagQuestionCount(
      isIncrement: boolean,
      tags: string[],
      dbSuffix: string,
  ) {
  ...
  static async addNotification(
      ...
      url: string = '',
      type: string,
      timestamp: any = admin.firestore.FieldValue.serverTimestamp(),
    ...
    if (senderId == recipientId) {
      console.log(
          'senderid and recipientId are the same ignore this notification',
      );
      return;
    }
    ...
    await admin
        ...
        .doc(recipientId)
        .collection(`IbNotifications${dbSuffix}`)
        .doc(id)
        .set(
            {
              id: id,
      ...
      try {
        ...
        if (userDoc.exists) {
          ...
          const timestamp: { [key: string]: number } =
            userDoc.data()!.fcmTokenTimestamp;
          ...
          if (days > 180) {
            ...
            await admin
                .messaging()
                .unsubscribeFromTopic(token, `Users${dbSuffix}`);
          ...
          let notificationCount: number | undefined =
            userDoc.data()!.notificationCount;
          notificationCount =
          ...
          const androidNotification = {
            ...
            channelId: 'icebr8k_custom_channel',
          };
   ...
   */
  static async handleMessageAdd(
      chatDoc: FirebaseFirestore.DocumentSnapshot<FirebaseFirestore.DocumentData>,
      lastMessage: FirebaseFirestore.DocumentData,
      dbSuffix: string,
  ) {
